...
module ram_lookahead_avm_tb;
    ...
    //------------------------------------------------------------------------
    // Behavioural Avalon byte memory behind the MEGA65 KFSDRAM overlay
    //------------------------------------------------------------------------
    // MEGA65: the SDRAM pins now carry a byte-wide Avalon-style bus (see
    // CORE/rtl/overlay/KFSDRAM.sv). The memory keeps the upstream layout, one
    // byte per 16-bit word indexed by the low 13 address bits, so the rest of
    // this bench is unchanged. waitrequest is re-drawn every cycle (one in
    // three stalls) and read data returns through a 3-stage pipeline, in order.
    ...
    logic       avm_waitrequest   = 1'b0;
    logic       avm_readdatavalid;
    logic [7:0] avm_readdata;
    wire        avm_accept        = (avm_read | avm_write) & ~avm_waitrequest;

    integer sdram_reads     = 0;    // accepted byte reads  (one per upstream CAS)
    integer sdram_writes    = 0;    // accepted byte writes
    integer sdram_activates = 0;    // bursts: one per request assertion edge

    logic       req_d = 1'b0;
    logic [7:0] rd_pipe0, rd_pipe1, rd_pipe2;
    logic       rv_pipe0 = 0, rv_pipe1 = 0, rv_pipe2 = 0;

    always @(posedge clock) begin
        req_d <= avm_read | avm_write;
        if ((avm_read | avm_write) & ~req_d)
            sdram_activates <= sdram_activates + 1;

        avm_waitrequest <= (($urandom % 3) == 0);

        rv_pipe0 <= 1'b0;
        if (avm_accept && avm_read) begin
            rd_pipe0    <= mem[avm_address[12:0]][7:0];
            rv_pipe0    <= 1'b1;
            sdram_reads <= sdram_reads + 1;
        end
        if (avm_accept && avm_write) begin
            mem[avm_address[12:0]][7:0] <= avm_writedata;
            sdram_writes <= sdram_writes + 1;
        end
        {rd_pipe1, rv_pipe1} <= {rd_pipe0, rv_pipe0};
        {rd_pipe2, rv_pipe2} <= {rd_pipe1, rv_pipe1};
    end

    assign avm_readdata      = rd_pipe2;
    assign avm_readdatavalid = rv_pipe2;
    assign sdram_dq_in       = {6'b0, avm_readdatavalid, avm_waitrequest, avm_readdata};
