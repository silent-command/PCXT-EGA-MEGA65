# xsim run of analog_pipeline_350_tb: the REAL framework analog (VGA) path of this port -
...
# (video_overlay / vga_recover_counters / vga_osm / ram_init) - driven with the 350-line EGA raster (16.257 MHz NCO dot enable)
# and measured at the VGA pins. See the header of analog_pipeline_350_tb.sv and docs/analog-video.md.
#   powershell -File run_analog_pipeline_350_tb.ps1
# Full log: CORE/ooc/analog_pipeline_350_tb/run.log
...
$work   = Join-Path $core 'ooc\analog_pipeline_350_tb'
...
if ($LASTEXITCODE -ne 0) { Write-Output "A350 RESULT: FAIL (MiSTer Verilog does not compile)"; exit 1 }
...
$apSim = $apSrc
$apSim = $apSim -replace '(?m)^   component video_mixer is', "   -- xsim: port-map conversions of analog_pipeline.vhd:141-143 moved here (see run_analog_pipeline_350_tb.ps1)`n   signal video_red_u        : unsigned(7 downto 0);`n   signal video_green_u      : unsigned(7 downto 0);`n   signal video_blue_u       : unsigned(7 downto 0);`n`n   component video_mixer is"
...
if ($n1 -ne 3 -or $n2 -ne 3 -or $n3 -ne 3 -or $apSim -notmatch 'signal video_red_u') {
    Write-Output "A350 RESULT: FAIL (analog_pipeline.vhd transform for xsim did not apply as expected: $n1/$n2/$n3)"; exit 1
}
Set-Content -Path (Join-Path $work 'analog_pipeline_350_sim.vhd') -Value $apSim -Encoding ascii

# framework VHDL + the port's control block
& "$bin\xvhdl.bat" -2008 "$m2m\ram_init.vhd" "$av\vga_osm.vhd" "$av\vga_recover_counters.vhd" "$av\video_overlay.vhd" "$core\vhdl\analog_line_doubler.vhd" "$work\analog_pipeline_350_sim.vhd" "$core\vhdl\analog_video_ctl.vhd" "$here\analog_pipeline_wrap.vhd" 2>&1 | Select-String -Pattern 'ERROR' | ForEach-Object { $_.Line }
if ($LASTEXITCODE -ne 0) { Write-Output "A350 RESULT: FAIL (framework VHDL does not compile)"; exit 1 }

# bench
& "$bin\xvlog.bat" -sv "$here\analog_pipeline_350_tb.sv" 2>&1 | Select-String -Pattern 'ERROR' | ForEach-Object { $_.Line }
if ($LASTEXITCODE -ne 0) { Write-Output "A350 RESULT: FAIL (bench does not compile)"; exit 1 }
& "$bin\xvlog.bat" "$vivado\data\verilog\src\glbl.v" | Out-Null

$font = (Join-Path $root 'M2M\font\Anikki-16x16-m2m.rom') -replace '\\', '/'
& "$bin\xelab.bat" -L xpm -debug off -generic_top "`"FONT_FILE=$font`"" analog_pipeline_350_tb glbl -s analog_pipeline_350_sim 2>&1 | Select-String -Pattern 'ERROR' | ForEach-Object { $_.Line }
if ($LASTEXITCODE -ne 0) { Write-Output "A350 RESULT: FAIL (elaboration failed)"; exit 1 }

& "$bin\xsim.bat" analog_pipeline_350_sim -R 2>&1 | Tee-Object -FilePath run.log | Select-String -Pattern 'A350|RESULT|CSYNC|ERR|Error|Failure|FATAL' | ForEach-Object { $_.Line }
