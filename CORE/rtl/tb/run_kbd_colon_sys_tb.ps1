# xsim run of kbd_colon_sys_tb: the ":" -> ";" bench. The REAL MCL86 8088 running the REAL 8088 BIOS
# (bios-xt.bin) on the real bus arbiter / ready / PIC / PIT / PPI / KFPS2KB and
# the real MEGA65 PS/2 keyboard device; types "*", Shift+"*", "a:", "c:" ... and checks the BIOS
# keyboard buffer at 0040:001E.
#
#   powershell -File run_kbd_colon_sys_tb.ps1              # 4.77 MHz
#   powershell -File run_kbd_colon_sys_tb.ps1 -Max         # "Max" CPU speed setting
#   powershell -File run_kbd_colon_sys_tb.ps1 -Upstream    # upstream chipset RTL instead of the CORE/rtl/overlay files
#   powershell -File run_kbd_colon_sys_tb.ps1 -KbdVhdl <dir>  # take keyboard.vhd + ps2_tx.vhd from <dir> (e.g. the pre-fix files, for the repro)
#   powershell -File run_kbd_colon_sys_tb.ps1 -NoPatch     # keep the beepinit delay
#   powershell -File run_kbd_colon_sys_tb.ps1 -WorkDir <dir>  # work directory (default CORE/ooc/kbd_colon_sys_tb)
#
# Full log: <work>/run.log
param([switch]$Max, [switch]$Upstream, [switch]$NoPatch, [string]$KbdVhdl = "", [string]$WorkDir = "")
...
function Pick($name, $dir) {
    ...
    if (-not $Upstream -and (Test-Path $o)) { return $o }
...
$sv = @(
    (Join-Path $here 'kbd_colon_sys_tb.sv'),
...
if ($KbdVhdl -ne '') { $vhdl = Resolve-Path $KbdVhdl }   # (parameter names must not collide with $vhdl / $work: PowerShell is case-insensitive)
$tag = if ($Upstream) { 'upstream chipset RTL' } else { 'overlay chipset RTL (as built)' }
$tag = "$tag, keyboard.vhd from $vhdl"
Write-Output "=== kbd_colon_sys_tb : $tag ==="

$work = if ($WorkDir -ne '') { $WorkDir } else { Join-Path $core 'ooc\kbd_colon_sys_tb' }
New-Item -ItemType Directory -Force $work | Out-Null
$work = Resolve-Path $work
Set-Location $work
...
& "$bin\xelab.bat" -debug typical work.kbd_colon_sys_tb -s kbd_colon_sys_sim 2>&1 | Select-String -Pattern 'ERROR' | ForEach-Object { $_.Line }
...
& "$bin\xsim.bat" kbd_colon_sys_sim -R @plus 2>&1 | Tee-Object -FilePath run.log |
    Select-String -Pattern 'RESULT|---|IN 60h|IMR|\*\*\*|CHECK FAILED|observations|values|final:|running|patch|loaded|reset|unmasked|HLT|BDA|RING|ring after' | ForEach-Object { $_.Line }
