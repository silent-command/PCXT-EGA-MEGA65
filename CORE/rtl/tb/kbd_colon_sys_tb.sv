...
// kbd_colon_sys_tb.sv
//
// The ":" -> ";" report: typing the MEGA65's ":" key sometimes gives ";" in
// DOS, once, and the next presses are right.  The board repro is "*", then
// Shift+"*" (= "}"), then "*" again, which prints "8".  This bench types those
// sequences, and "a:" / "c:" the way a human does (rollover, fast taps, a
// capital letter first), through the whole keyboard path with the REAL 8088
// BIOS's INT 9 and checks what lands in the BIOS keyboard buffer at 0040:001E.
//
// Same machine as kbd_bios_sys_tb.sv:
//
...
//   * 64 KB of zero-wait RAM, B8000 text RAM, and the BIOS image at FC000
//     (SW/8088_bios/binaries/bios-xt.bin, the MACHINE_XT build).
//
// Once the POST has unmasked the PIC (bios.asm:1097) and the keyboard's AA has
// been serviced, the BIOS spends the rest of its POST (the floppy detection,
// with no FDC here) with interrupts enabled and nothing reading the keyboard
// buffer: INT 9 runs on every scancode exactly as at a DOS prompt and the ring
// keeps everything it stores.  The scenarios fill 14 of its 15 usable slots.
//
// Sim-only ROM tweaks as in kbd_bios_sys_tb (off with -testplusarg NOPATCH):
// pure delay loops of the POST are shortened.
//
//   xsim kbd_colon_sys_sim -R                       4.77 MHz
//   xsim kbd_colon_sys_sim -R -testplusarg MAXSPEED  "Max" CPU speed setting
//
// Run with run_kbd_colon_sys_tb.ps1 (Vivado xsim, mixed language).

`timescale 1ns/1ps

module kbd_colon_sys_tb;
    ...
    // ==================================================================
    //  The BIOS keyboard buffer (ring at 0040:001E, head 40:1A, tail 40:1C)
    // ==================================================================
    // Nothing consumes the ring here, so head stays at 001Eh and the entries
    // are word i at 0040:001E + 2i: AL = ASCII, AH = BIOS scan code.
    integer      ring_expected = 0;                  // entries expected so far
    integer      ring_checked  = 0;                  // entries already checked
    logic [15:0] ring_want [0:15];
    string       ring_what [0:15];

    function automatic [15:0] ring_word(input int i);
        return {mem[20'h41F + 2*i], mem[20'h41E + 2*i]};
    endfunction
    function automatic int ring_entries();
        int head = {mem[20'h41B], mem[20'h41A]};
        int tail = {mem[20'h41D], mem[20'h41C]};
        return (tail >= head) ? (tail - head) / 2 : (tail + 32 - head) / 2;
    endfunction
    function automatic string printable(input [7:0] c);
        return (c >= 8'h20 && c < 8'h7F) ? $sformatf("%c", c) : ".";
    endfunction
    task automatic expect_entry(input [15:0] w, input string what);
        ring_want[ring_expected] = w; ring_what[ring_expected] = what; ring_expected++;
    endtask
    task automatic check_ring(input string scenario);
        $write("    ring after %s (%0d entries):", scenario, ring_entries());
        for (int i = 0; i < ring_entries(); i++) $write(" %04x'%s'", ring_word(i), printable(ring_word(i)[7:0]));
        $display("");
        check(ring_entries() == ring_expected, $sformatf("%s: %0d entries in the ring, expected %0d", scenario, ring_entries(), ring_expected));
        for (int i = ring_checked; i < ring_expected && i < ring_entries(); i++)
            check(ring_word(i) == ring_want[i], $sformatf("%s: entry %0d is %04x '%s', expected %04x (%s)",
                  scenario, i, ring_word(i), printable(ring_word(i)[7:0]), ring_want[i], ring_what[i]));
        ring_checked = ring_expected;
    endtask

    // ==================================================================
    //  Test
    // ==================================================================
    localparam M65_A = 10, M65_C = 20, M65_RETURN = 1, M65_COLON = 45, M65_ASTERISK = 49, M65_LSHIFT = 15, M65_RSHIFT = 52;
    integer base;
    real t_unmask;

    // one key tap: down for hold_ms, then a gap
    task automatic tap(input int k, input real hold_ms = 25.0, input real gap_ms = 25.0);
        pressed[k] = 1'b1; #(hold_ms * 1ms); pressed[k] = 1'b0; #(gap_ms * 1ms);
    endtask

    initial begin
        $display("=== kbd_colon_sys_tb: real MCL86 running %s, CPU speed select %0d ===", BIOS_FILE, clk_select);
        ...
        // wait for the BIOS to unmask the PIC (bios.asm:1097), then for the
        // keyboard's AA reply to be serviced and the POST beeps to be over
        wait (imr_unmasked);
        t_unmask = $realtime;
        $display("  [%0t] PIC unmasked; port B = %02x", $time, port_b_out);
        #150ms;
        check(find_code(8'hAA, 0) >= 0, "INT 9 serviced the keyboard's AA self-test reply");
        check(ring_entries() == 0, "keyboard buffer empty before typing");
        base = p60_count;

        $display("--- S1: the board repro: '*', Shift+'*' (key released before the Shift), '*', '*'");
        tap(M65_ASTERISK);
        pressed[M65_LSHIFT] = 1'b1; #25ms; tap(M65_ASTERISK); pressed[M65_LSHIFT] = 1'b0; #25ms;
        tap(M65_ASTERISK);
        tap(M65_ASTERISK);
        expect_entry(16'h092A, "*");
        expect_entry(16'h1B7D, "} = Shift+]");
        expect_entry(16'h092A, "* right after the Shift use (the board typed 8)");
        expect_entry(16'h092A, "*");
        #30ms; check_ring("S1");

        $display("--- S2: 'a:' with rollover (':' pressed while 'a' is still down)");
        pressed[M65_A] = 1'b1; #30ms; pressed[M65_COLON] = 1'b1; #20ms; pressed[M65_A] = 1'b0; #20ms; pressed[M65_COLON] = 1'b0; #30ms;
        expect_entry(16'h1E61, "a");
        expect_entry(16'h273A, ": (rollover)");
        check_ring("S2");

        $display("--- S3: 'c:' typed fast (15 ms taps, 10 ms apart)");
        tap(M65_C, 15, 10); tap(M65_COLON, 15, 30);
        expect_entry(16'h2E63, "c");
        expect_entry(16'h273A, ": (fast)");
        check_ring("S3");

        $display("--- S4: 'A:' - Shift held for the letter and released, then ':'");
        pressed[M65_LSHIFT] = 1'b1; #20ms; tap(M65_A, 20, 20); pressed[M65_LSHIFT] = 1'b0; #20ms; tap(M65_COLON, 20, 30);
        expect_entry(16'h1E41, "A");
        expect_entry(16'h273A, ": after a Shift use (the report's ;)");
        check_ring("S4");

        $display("--- S5: 'C:' with the right Shift, ':' 5 ms after the Shift release");
        pressed[M65_RSHIFT] = 1'b1; #15ms; tap(M65_C, 15, 5); pressed[M65_RSHIFT] = 1'b0; #5ms; tap(M65_COLON, 15, 30);
        expect_entry(16'h2E43, "C");
        expect_entry(16'h273A, ": after a right Shift use");
        check_ring("S5");

        $display("--- S6: ':' then Enter in rollover (Enter pressed while ':' is still down)");
        pressed[M65_COLON] = 1'b1; #20ms; pressed[M65_RETURN] = 1'b1; #10ms; pressed[M65_COLON] = 1'b0; #10ms; pressed[M65_RETURN] = 1'b0; #40ms;
        expect_entry(16'h273A, ":");
        expect_entry(16'h1C0D, "Enter");
        check_ring("S6");

        dump_bda();
        $display("--- observations: port 60h reads=%0d (%0d while typing) port 61h writes=%0d INTA cycles=%0d HLTs=%0d",
                 p60_count, p60_count - base, p61_writes, inta_count, halts);
        $write("    port 60h values while typing:");
        for (int i = base; i < p60_count; i++) $write(" %02x", p60_codes[i]);
        $display("");
        ...
        check(keybord_irq == 1'b0, "KFPS2KB irq not left pending");
