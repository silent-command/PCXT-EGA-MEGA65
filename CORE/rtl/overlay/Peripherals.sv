...
module PERIPHERALS #(
        ...
        ,
        // NE1000 Ethernet card (MEGA65 port, CORE/rtl/ne1000.sv): I/O 320h..33Fh, IRQ 5 via ne1000_irq.
        // The MAC (CORE/vhdl/eth_mac.vhd) lives at the top level with the PHY pins; these are its
        // chipset-clock FIFO streams (9-bit entries, bit 8 = end of frame).
    ...
    // NE1000 at 320h..33Fh (registers 320h..32Fh, data port 330h..337h, reset port 338h..33Fh). The
    // card itself decides on ne1000_en; the select stays independent of it so that a disabled card
    // answers FFh like an empty slot (a driver probing 320h must see "no card", not 00h). Note the
    // overlap with the MPU-401 at 330h..331h: mpu401_chip_select wins in the read mux below and the
    // MPU-401 is disabled in this port (main.vhd osm_mpu401_disable_i => '1'), so the two never
    // coexist; if the MPU-401 ever comes back, the card must move (e.g. 280h) or the MPU-401 to 300h.
    wire    ne1000_chip_select      = (iorq && ~address_enable_n && ({address[15:5], 5'd0} == 16'h0320)); // 0x320 .. 0x33F
...
//        .io_wait        (),
    ...
    //
    // NE1000 Ethernet card (MEGA65 port). Registered read data, trailing-edge writes, no READY use.
    //
    logic   [7:0]   ne1000_readdata;

    ne1000 ne1000_card (
        .clk            (clock),
        .reset          (reset),
        .enable         (ne1000_en),
        .cs             (ne1000_chip_select),
        .address        (address[4:0]),
        .io_read_n      (io_read_n),
        .io_write_n     (io_write_n),
        .data_in        (internal_data_bus),
        .data_out       (ne1000_readdata),
        .irq            (ne1000_irq),
        .mac_addr       (ne1000_mac),
        .rx_empty       (eth_rx_empty),
        .rx_rd          (eth_rx_rd),
        .rx_data        (eth_rx_data),
        .tx_full        (eth_tx_full),
        .tx_wr          (eth_tx_wr),
    ...
    begin
        ...
        else if (ne1000_chip_select && (~io_read_n))
        begin
            data_bus_out_from_chipset <= 1'b1;
            data_bus_out <= ne1000_readdata;                 // FFh when the card is disabled
        end
