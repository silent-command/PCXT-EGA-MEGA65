...
module RAM (
    ...
    // MEGA65 overlay: the memory behind KFSDRAM is HyperRAM through the
    // framework arbiter (17..65 chipset clocks per byte, not 7), so the open
    // loop bet is lost at every CPU speed. Always run the real handshake.
    wire    strict_ready = 1'b1;
