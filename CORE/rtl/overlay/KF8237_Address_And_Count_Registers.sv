...
//
// MEGA65 overlay (timing): the "Reads Registers" mux selects on the new
// address_in input instead of the read_flag-qualified one-hot
// read_current_address / read_current_word_count vectors, so the late
// io_read_n / chip_select_n qualification no longer sits in front of the
// register tree.  read_address_or_count is identical to upstream whenever
// any read_current_* bit is set (those bits are exactly
// read_flag & decode(address_in)); when none is set the value is not
// consumed (KF8237.sv gates data_bus_out on read_flag).  Everything else,
// including the byte-pointer logic driven by read_current_*, is unchanged.
// Instantiated only from the overlaid KF8237.sv.
//
...
module KF8237_Address_And_Count_Registers (
    ...
    input   logic   [3:0]   address_in,             // MEGA65 overlay: bus address for the read mux
    ...
    // MEGA65 overlay: address-indexed select.  Upstream's one-hot chain was
    //   read_current_address[i]    = read_flag & (address_in == {1'b0, i, 1'b0})
    //   read_current_word_count[i] = read_flag & (address_in == {1'b0, i, 1'b1})
    // so with read_flag = 1 the selected register is current_address[a[2:1]]
    // for even a[3:0] < 8, current_word_count[a[2:1]] for odd a < 8, and
    // 16'h00 for a >= 8 -- exactly the mux below.
    always_comb begin
        if (address_in[3])                      read_register = 16'h00;
        else if (address_in[0])                 read_register = current_word_count[address_in[2:1]];
        else                                    read_register = current_address[address_in[2:1]];
