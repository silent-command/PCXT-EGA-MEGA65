// MEGA65 overlay of rtl/video/ega_vram.v. Only the generic (non-Altera) memory
// path differs: each plane is described as one true dual-port RAM, port A
// (clk) sharing a single muxed address for CPU reads and writes with the
// read suppressed during a write (altsyncram rden_a = cpu_access & ~write,
// NO_CHANGE), port B (clk_vram) read-only. The upstream generic path uses
// separate read and write addresses on port A, which makes Vivado duplicate
// every plane to get a third port: 128 RAMB36 instead of 64 for 256 KB.
// This matches the altsyncram configuration the core ships with on MiSTer.
//============================================================================
//
...
`ifndef ALTERA_RESERVED_QIS
    (* ramstyle = "M10K", ram_style = "block" *) reg [7:0] plane0[0:EGA_PLANE_DEPTH-1];
    (* ramstyle = "M10K", ram_style = "block" *) reg [7:0] plane1[0:EGA_PLANE_DEPTH-1];
    (* ramstyle = "M10K", ram_style = "block" *) reg [7:0] plane2[0:EGA_PLANE_DEPTH-1];
    (* ramstyle = "M10K", ram_style = "block" *) reg [7:0] plane3[0:EGA_PLANE_DEPTH-1];
`endif
...
`ifdef ALTERA_RESERVED_QIS
    ...
    defparam
        ...
        plane0_ram.read_during_write_mode_mixed_ports = "OLD_DATA",
    ...
    defparam
        ...
        plane1_ram.read_during_write_mode_mixed_ports = "OLD_DATA",
    ...
    defparam
        ...
        plane2_ram.read_during_write_mode_mixed_ports = "OLD_DATA",
    ...
    defparam
        ...
        plane3_ram.read_during_write_mode_mixed_ports = "OLD_DATA",
...
`else
    always @(posedge clk) begin
        if (write_plane0_commit)
            plane0[plane0_cpu_addr] <= cpu_plane0_new;
        else if (cpu_access)
            cpu_plane0_q <= plane0[plane0_cpu_addr];
    end

    always @(posedge clk) begin
        if (write_plane1_commit)
            plane1[plane1_cpu_addr] <= cpu_plane1_new;
        else if (cpu_access)
            cpu_plane1_q <= plane1[plane1_cpu_addr];
    end

    always @(posedge clk) begin
        if (write_plane2_commit)
            plane2[plane2_cpu_addr] <= cpu_plane2_new;
        else if (cpu_access)
            cpu_plane2_q <= plane2[plane2_cpu_addr];
    end

    always @(posedge clk) begin
        if (write_plane3_commit)
            plane3[plane3_cpu_addr] <= cpu_plane3_new;
        else if (cpu_access)
            cpu_plane3_q <= plane3[plane3_cpu_addr];
    end
`endif
