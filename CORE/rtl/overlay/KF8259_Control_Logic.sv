...
//
// MEGA65 overlay: ICW1 also clears the in-service register, as the 8259A
// does when it is re-initialised.
//
// IBM-style BIOSes reboot on Ctrl+Alt+Del from inside INT 9 with the IRQ1
// service still open: no EOI, `mov word [40:72],1234h` and a far jump to the
// POST entry (skiselev 8088_bios keyboard.inc:329-331, Super PC/Turbo XT BIOS
// pcxtbios.asm:2267-2269, IBM 5160 KB_INT). They rely on the POST's ICW1 to
// reset the controller. Upstream KF8259 clears the request latches on ICW1
// (clear_interrupt_request below) but leaves in_service_register alone, so
// after the warm POST ISR bit 1 stays set, the priority resolver never lets
// IRQ1 through again, the keyboard's first byte after the reset (the AA of the
// self test) is never read by INT 9, KFPS2KB holds irq high with the PS/2
// clock inhibited, and the keyboard is dead until the next cold POST. The
// same happens with any interrupt whose handler restarts the machine.
//
// end_of_interrupt is the only path that clears in_service_register
// (KF8259_In_Service.sv), so ICW1 drives it with all ones here.
//
...
module KF8259_Control_Logic (
    ...
    always_comb begin
        if (write_initial_command_word_1 == 1'b1)
            end_of_interrupt = 8'b11111111;     // MEGA65 overlay: ICW1 clears the ISR
