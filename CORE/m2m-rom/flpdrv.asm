...
;
; flpdrv.asm: the QNICE side of the read path of docs/floppy.md (phase 2).
;
...
; geometry it detects (1.44 MB or 720 KB, read-only), and every block the
; emulated FDC requests for vdrive 0 is served by the sector engine in
; CORE/vhdl/floppy_sector_engine.vhd through the register window
; 0xFFFD of the PCXT ROM device (CORE/vhdl/rom_loader.vhd, see there for
; the register map).
;
; States (FLP_STATE):
...
; Writes never reach here (the mount is read-only: floppy.v refuses them
; with a write-protect error); FLP_DRV_WR only acknowledges, in case.
;
...
FLP_STR_HD      .ASCII_W " -> 1.44 MB, mounted read-only"
FLP_STR_DD      .ASCII_W " -> 720 KB, mounted read-only"
...
FLP_STR_WR      .ASCII_W "FLP: write request dropped (read-only) lba="
...
; levels are remembered in FLP_CTRL, the clear bit is a pulse
FLP_CTRL_SET    INCRB
                MOVE    R8, R0
                MOVE    R0, R9
                AND     0xFFFD, R9              ; without the clear pulse
                MOVE    FLP_CTRL, R1
                MOVE    R9, @R1
                MOVE    R0, R9
                MOVE    FLP_R_CTRL, R8
...
; (VD_STROBE_IM: size, read-only) and put "Internal drive" on the menu line
...
; FLP_ACK: R8 = 1 error / 0 data ready: set the block-error flag for
; mgmt_bridge, then strobe the vdrive acknowledge (held a few instructions
; so that the bridge synchroniser sees it high)
FLP_ACK         INCRB
                MOVE    FLP_CTL_EN, R0
                CMP     0, R8
                RBRA    _FLP_ACK_1, Z
...
; FLP_DRV_WR: R8 = vdrive: a write request cannot happen (read-only mount);
; acknowledge it without storing anything
